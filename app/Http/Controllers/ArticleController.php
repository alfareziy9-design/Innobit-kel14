...
class ArticleController extends Controller
{
    ...
    public function uploadContentMedia(Request $request)
    {
        ...
        if (! $path) {
            throw new RuntimeException('Media gagal disimpan.');
        }
    ...
    private function normalizedContentImageSource(string $src): ?string
    {
        ...
        $storageContentFolder = '/storage/uploads/artikel/content/';
        $legacyContentFolder = '/uploads/artikel/content/';
        $folderPosition = stripos($path, $storageContentFolder);
        $normalizedPrefix = $storageContentFolder;

        if ($folderPosition === false) {
            $folderPosition = stripos($path, $legacyContentFolder);
            $normalizedPrefix = $legacyContentFolder;
        }

        if ($folderPosition === false && str_starts_with(ltrim($path, '/'), 'storage/uploads/artikel/content/')) {
            $path = '/'.ltrim($path, '/');
            $folderPosition = 0;
            $normalizedPrefix = $storageContentFolder;
        } elseif ($folderPosition === false && str_starts_with(ltrim($path, '/'), 'uploads/artikel/content/')) {
            $path = '/'.ltrim($path, '/');
            $folderPosition = 0;
            $normalizedPrefix = $legacyContentFolder;
        }
        ...
        if (! preg_match(
            '~^'.preg_quote($normalizedPrefix, '~').'[^?#]+\.(gif|jpe?g|png|webp)$~i',
            $normalizedPath
        )) {
            return null;
        }
