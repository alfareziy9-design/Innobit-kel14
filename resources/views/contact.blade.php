    ...
    <div class="grid md:grid-cols-2 gap-8">
        <div class="bg-white rounded-2xl shadow-sm border p-6 md:p-8">
            ...
            <form method="POST" action="{{ route('contact.send') }}">
                @csrf
                <div class="mb-4">
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Nama Lengkap" class="w-full border rounded-none px-4 py-3 focus:outline-none focus:ring-2 focus:ring-lime-500">
                </div>
                <div class="mb-4">
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="Alamat Email" class="w-full border rounded-none px-4 py-3 focus:outline-none focus:ring-2 focus:ring-lime-500">
                </div>
                <div class="mb-6">
                    <textarea name="message" rows="5" placeholder="Pesan Anda..." class="w-full border rounded-none px-4 py-3 focus:outline-none focus:ring-2 focus:ring-lime-500">{{ old('message') }}</textarea>
                </div>
