...
@section('title', $category->name.' - InnoBit')
...
<div class="min-h-screen bg-lime-50/45 text-slate-950">
    <main class="mx-auto max-w-7xl px-4 py-8 lg:py-10">
        ...
        <header class="mb-8 mt-5">
            ...
            <h1 class="mt-2 text-3xl font-black tracking-tight md:text-4xl">{{ $category->name }}</h1>
            <p class="mt-2 text-slate-600">{{ $articles->total() }} artikel published dalam kategori ini.</p>
        </header>
        ...
        @else
            <div class="rounded-lg border border-dashed border-lime-300 bg-white p-12 text-center">
                <h2 class="text-xl font-black text-slate-800">Belum ada artikel</h2>
                <p class="mt-2 text-slate-500">Belum ada artikel published dalam kategori {{ $category->name }}.</p>
            </div>
        @endif
    </main>
</div>
